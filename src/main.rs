...
use std::fs::{read_dir, write as write_to_file, File, canonicalize as to_absolute_path};
...
fn get_avatars(path: &str, current: String) -> Avatars {
	let mut avatars: Vec<String> = read_dir(path)
		...
		.map(|y| String::from(to_absolute_path(&y).unwrap_or_else(|_| panic!("Couldn't convert \"{}\" to absolute path", y.to_str().unwrap())).to_str().unwrap()))
		.collect();
	if avatars.len() < 2 {
		panic!(
